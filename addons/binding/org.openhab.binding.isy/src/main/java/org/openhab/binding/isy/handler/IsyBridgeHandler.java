...
public class IsyBridgeHandler extends BaseBridgeHandler implements InsteonClientProvider {
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        logger.debug("isy bridge handler called, channel: {}, command: {}", channelUID, command);
        if (IsyBindingConstants.CHANNEL_BRIDGE_INITIATE_RECONNECT.equals(channelUID.getId())
                && OnOffType.ON.equals(command)) {
            initialize();
            updateState(IsyBindingConstants.CHANNEL_BRIDGE_INITIATE_RECONNECT, OnOffType.OFF);
        }
        ...
        eventSubscriber = new IsyWebSocketSubscription(config.getIpAddress(), authorizationHeaderValue,
                new ISYModelChangeListener() {
                    ...
                    public void onModelChanged(Event event) {
                        updateState(IsyBindingConstants.CHANNEL_BRIDGE_LAST_UPDATE, new DateTimeType());
